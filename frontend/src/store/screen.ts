...
import type { Tick, OrderBook, GridResult, ScreenConfig } from '@/types'

// 错误信息统一从后端 detail 中提取（enable 时 detail 为 {message, items}）
function extractError(e: any): { message: string; items?: string[] } {
  const detail = e?.response?.data?.detail
  if (detail && typeof detail === 'object') return { message: detail.message || '大屏启用失败', items: detail.items || [] }
...
export const useScreenStore = defineStore('screen', () => {
  ...
  const report = ref<GridResult | null>(null)
  const ticks = ref<Tick[]>([])
  const orderBook = ref<OrderBook | null>(null)
  const wsConnected = ref(false)
  const loginError = ref('')
  const enableError = ref<{ message: string; items: string[] } | null>(null)
  ...
  async function fetchConfig() {
    const { data } = await axios.get<ScreenConfig>('/api/screen/config')
  ...
  async function enable() {
    enableError.value = null
    const items: string[] = []
    if (!enablePassword.value.trim()) items.push('口令为空：必须设置非空口令')
    const names = enableAccountsText.value.split(/[\n,，;；]/).map(s => s.trim()).filter(Boolean)
    const valid: string[] = []
    const invalid: string[] = []
    for (const n of names) (/\s/.test(n) ? invalid : valid).push(n)
    if (!valid.length) items.push('权限配置缺失：至少需要一个已授权账号')
    if (invalid.length) items.push('授权账号存在不合格项（账号不能包含空白字符）：' + [...new Set(invalid)].join('、'))
    if (items.length) {
  ...
  // 授权账号登录
  async function login(loginAccount: string, password: string) {
    loginError.value = ''
    try {
  ...
  async function loadSnapshot() {
    try {
      ...
      if (data.ticks) ticks.value = data.ticks.slice(-60)
      if (data.orderBook) orderBook.value = data.orderBook
    } catch (e: any) {
  ...
  function connectWS() {
    ...
    ws.onclose = (ev) => {
      ...
      if (active.value && ev.code === 1008) deniedReason.value = ev.reason || '访问被拒绝：未授权或登录已失效'
    }
  }
  ...
  async function exit() {
    ...
    disconnectWS()
    if (t) {
      try { await axios.post('/api/screen/exit', null, { headers: { Authorization: `Bearer ${t}` } }) } catch { /* 令牌可能已失效，忽略 */ }
    }
    token.value = ''
    account.value = ''
    ticks.value = []
